...
const SubmitQuote = ({ showNotification, navigateTo, viewParam: jobId, currentUser }) => {
    ...
    if (isLoading) return <div className="text-center p-10">Pagina laden...</div>;
        ...
        <div className="max-w-2xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="card bg-base-100 shadow-xl p-8 space-y-6">
                ...
                <div>
                    ...
                    <textarea id="comments" name="comments" placeholder="Extra informatie over uw offerte..." value={formData.comments} onChange={handleChange} className="textarea textarea-bordered w-full" rows="4"></textarea>
                </div>
                <button type="submit" disabled={isSubmitting} className="w-full btn btn-primary">
                    {isSubmitting ? <span className="loading loading-spinner"></span> : 'Offerte Indienen'}
                </button>
            </form>
        </div>
    );
